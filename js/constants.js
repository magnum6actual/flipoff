// Defaults - overridden by config.json if present
const DEFAULTS = {
  grid: { cols: 22, rows: 5 },
  timing: {
    scrambleDuration: 800,
    flipDuration: 300,
    staggerDelay: 25,
    totalTransition: 3800,
    messageInterval: 4000,
  },
  charset: 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,-!?\'/: ',
  scrambleColors: [
    '#00AAFF', '#00FFCC', '#AA00FF',
    '#FF2D00', '#FFCC00', '#FFFFFF',
  ],
  accentColors: [
    '#00FF7F', '#FF4D00', '#AA00FF',
    '#00AAFF', '#00FFCC',
  ],
  messages: [
    ['', 'GOD IS IN', 'THE DETAILS .', '- LUDWIG MIES', ''],
    ['', 'STAY HUNGRY', 'STAY FOOLISH', '- STEVE JOBS', ''],
    ['', 'GOOD DESIGN IS', 'GOOD BUSINESS', '- THOMAS WATSON', ''],
    ['', 'LESS IS MORE', '', '- MIES VAN DER ROHE', ''],
    ['', 'MAKE IT SIMPLE', 'BUT SIGNIFICANT', '- DON DRAPER', ''],
    ['', 'HAVE NO FEAR OF', 'PERFECTION', '- SALVADOR DALI', ''],
  ],
};

let cfg = DEFAULTS;
try {
  const res = await fetch('config.json');
  if (res.ok) {
    const json = await res.json();
    cfg = { ...DEFAULTS, ...json, grid: { ...DEFAULTS.grid, ...json.grid }, timing: { ...DEFAULTS.timing, ...json.timing } };
  }
} catch {
  // config.json missing or invalid - use defaults
}

export const GRID_COLS = cfg.grid.cols;
export const GRID_ROWS = cfg.grid.rows;

export const SCRAMBLE_DURATION = cfg.timing.scrambleDuration;
export const FLIP_DURATION = cfg.timing.flipDuration;
export const STAGGER_DELAY = cfg.timing.staggerDelay;
export const TOTAL_TRANSITION = cfg.timing.totalTransition;
export const MESSAGE_INTERVAL = cfg.timing.messageInterval;

export const CHARSET = cfg.charset;
export const SCRAMBLE_COLORS = cfg.scrambleColors;
export const ACCENT_COLORS = cfg.accentColors;
export const MESSAGES = cfg.messages;
